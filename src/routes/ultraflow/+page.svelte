<script>
	...
	// let lastUpdated = moment().format('hh:mm:ss a');
	let lastUpdated;
	...
	onMount(async () => {
			...
			{},
			{
				set: (target, property, value) => {
					...
					if (property === 'tradeTS') {
						...
						if (tradeTime >= 150500 && tradeTime < 151000 && !target['playedAudio']) {
		...
		chartSettings = localStorage.getItem('chartSettingsUF');
		let storedSymbol = localStorage.getItem('aggSymbol');
		let storedAggType = localStorage.getItem('aggType');
		...
		if (!chartSettings) {
			...
			localStorage.setItem('chartSettingsUF', JSON.stringify(chartSettings));
		} else {
			chartSettings = JSON.parse(chartSettings);
		}
		aggType = storedAggType || aggType;
		symbol = storedSymbol || symbol;
		type = storedType || type;
		localStorage.removeItem('aggSymbol');
		localStorage.removeItem('aggType');
		...
		document.addEventListener('visibilitychange', function () {
			if (window.innerWidth <= 991) {
				if (!document.hidden) {
					...
					localStorage.setItem('aggSymbol', symbol);
					localStorage.setItem('aggType', aggType);
		...
		socket.onmessage = (e) => {
			...
			if (firstLoad && today === currentDate) {
				checkNonDataDays = 5;
	...
	const addPoint = (data) => {
		...
		let smartFlow = data[0];
		const ohlc = data[1];
		const putVol = data[2] || [];
		const callVol = data[3] || [];
		let optionsFlow = data[4] || [];
		...
		if (!historicalLoad) {
			...
			if (putVol.length === 1 && smartFlow.length === 1) {
				callVolSeries.pop();
				putVolSeries.pop();
			}

			if (putVol.length === 2 && smartFlow.length === 2) {
				callVolSeries.pop();
				putVolSeries.pop();
			}
		...
		for (let i = 0; i < smartFlow.length; i++) {
			...
			smartFlowSeries.push({ x: smartFlow[i][0], y: Number(smartFlow[i][1].toFixed(0)), color });
			...
			netStrengthBarSeries.push([
				ohlc[i][0],
				smartFlow[i][6] ? Number(smartFlow[i][6].toFixed(2)) : 0
			]);
	...
	const organizeSmartFlow = (data) => {
		chartConfig = createChartConfig(sfThreshold, manipulationThreshold);
		optionsFlowDesc = [];
		let smartFlowData = data[0];
		let ohlcData = data[1];
		let putVolData = data[2] || [];
		let callVolData = data[3] || [];
		let optionsFlow = data[4] || [];
		...
		// cleansedPutVolData.sort((a, b) => { return a[0]-b[0] });
		// cleansedCallVolData.sort((a, b) => { return a[0]-b[0] })
		...
		for (let i = 0; i < smartFlowData.length; i++) {
			...
			smartFlowSeed.push({
				x: smartFlowData[i][0],
				y: Number(smartFlowData[i][1].toFixed(0)),
				color
			});
			netStrengthBarSeed.push([
				smartFlowData[i][0],
				smartFlowData[i][6] ? Number(smartFlowData[i][6].toFixed(2)) : 0
			]);
			...
			volumeSeed.push({ x: Number(row[0]), y: Number(row[5]), color: volColor });
			if (
			...
			for (let j = 0; j < optionsFlow.length; j++) {
				...
				if (optionsFlow[j][1] > smartFlowData[i][0] || uaCounter === optionsFlow.length) {
					for (let key in ofHistogram) {
						ofSeed[key].push({ x: smartFlowData[i][0], y: ofHistogram[key] });
					}
		...
		lastUpdated.dateTime = new Date();
		// lastUpdated.tradeTS = smartFlowData[smartFlowData.length - 1][0];
			...
			<div class="filter-block">
				<div class="input-fields">
					<div class="checkbox-group">
						<div class="tabs style1 group">
							<div
								on:click={(e) => changeSymbol(e, 'SPY')}
								class="tab {aggType === 'SPY' ? 'active' : ''}"
							>
								SPY
							</div>
							<div
								on:click={(e) => changeSymbol(e, 'QQQ')}
								class="tab {aggType === 'QQQ' ? 'active' : ''}"
							>
								QQQ
							</div>
							<div
								on:click={(e) => changeSymbol(e, 'DIA')}
								class="tab {aggType === 'DIA' ? 'active' : ''}"
							>
								DIA
							</div>
						</div>
			...
			<div class="data-block">
				<div class="ranges-data">
					<div class="content-block">
						<br />
						<!-- <p><strong>Last Update: {lastUpdated}</strong><br /><span class="sub">Lorem ipsum dollar sit Amit:</span></p> -->
						<!-- <p class=""><span class="higlight-number shade1">157.90</span> USD <span class="sub red">-6% (-3%)</span></p> -->
						<p class=""><span class="higlight-number1">{aggType}</span></p>
					...
					<div class="live-updates-graph {circleAction}">
						...
						<div class="heading">
							<svg viewBox="0 0 200 200">
								...
								<text width="100" style="transform: translate(28px, 30px);">
									<textPath xlink:href="#curve" class="fill-class"> Ultra Flow </textPath>
								</text>
							</svg>
						</div>
						...
						<div class="heading">
							<svg viewBox="0 0 200 200">
								<path
									...
									id="curve1"
								/>
								<text width="100" style="transform: translate(42px, 28px) rotate(7deg);">
									<textPath xlink:href="#curve1" class="fill-class"> UF Tally </textPath>
								</text>
							</svg>
						</div>
						<div class="graph-block">
...
<ModelLayout enableClose title="Indicators" bind:hidden>
